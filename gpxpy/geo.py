...
# custom functions
def point_from_distance_bearing(location, distance, bearing):
    """ gets the coordinate of another point at a "distance" with "bearing"
    location: Location
    ...
    """
    ad = float(distance) / EARTH_RADIUS # angular distance in radians
    direction = to_rad(float(bearing))  # direction
    
    lat1 = to_rad(location.latitude)
    lon1 = to_rad(location.longitude)
    
    lat2 = mod_math.asin(mod_math.sin(lat1) * mod_math.cos(ad) + mod_math.cos(lat1) * mod_math.sin(ad) * mod_math.cos(direction))
    x = mod_math.cos(ad) - mod_math.sin(lat1) * mod_math.sin(lat2)
    y = mod_math.sin(direction) * mod_math.sin(ad) * mod_math.cos(lat1)
    lon2 = lon1 + mod_math.atan2(y, x)
    location2 = Location(to_deg(lat2), (to_deg(lon2)+540)%360-180)
    return location2 # normalise to −180..+180°


def midpoint_cartesian(location1, location2):
    """ gets the coordinate of midpoint of given two locations
    location1: cartesian points [lat1, lon1]
    location2: cartesian points [lat2, lon2]
    """
    return Location((location1[0] + location2[0]) / 2, (location1[1] + location2[1]) / 2)


def get_bearing(src, dst):
    """ gets the bearing from src to dst
    src: Location
    dst: Location

    source http://www.movable-type.co.uk/scripts/latlong.html
    """
    lat1 = to_rad(src.latitude)
    lon1 = to_rad(src.longitude)
    lat2 = to_rad(dst.latitude)
    lon2 = to_rad(dst.longitude)
    if ((lat1 == lat2) and (lon1 == lon2)):
        return None
    deltaLon = lon2 - lon1

    y = mod_math.sin(deltaLon) * mod_math.cos(lat2)
    x = mod_math.cos(lat1)* mod_math.sin(lat2) - mod_math.sin(lat1)* mod_math.cos(lat2) * mod_math.cos(deltaLon)
    theta = mod_math.atan2(y, x)

    return (to_deg(theta)+360) % 360
